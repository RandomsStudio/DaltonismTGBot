...
def send_photo(message):
    # bot.reply_to(message, f"Твоё сообщение содержит картинку. Сохраняю...")
    file_info = bot.get_file(message.photo[-1].file_id)
    file_name = file_info.file_path.split('/')[-1]
    downloaded_file = bot.download_file(file_info.file_path)
    with open(file_name, 'wb') as new_file:
        new_file.write(downloaded_file)
    # bot.reply_to(message, f"Сохранил!")
    result=get_class(model_path="keras_model.h5", labels_path="labels.txt", image_path=file_name)
    bot.reply_to(message, f"Результат: {result}")
    bot.reply_to(message, f"Анализирую цвета на вашем изображении...")
